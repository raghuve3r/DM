...
#table to fill manhattan distance between the users
mh_distance_table = [[0 for i in range(userCount)] for j in range(userCount)]

#filling the manhattan distance table between users
for x in range(userCount):
    for y in range(userCount):
        if x != y:
            num_array = numpy.array([user_table[x],numpy.array(user_table[y])])
            mh_distance_table[x][y] = float(str(pdist(num_array ,'cityblock')).replace('[','').replace(']',''))

#finding the similar users for a particular user
for x in range(userCount):
    for y in range(userCount):
        if mh_distance_table[x][y] > 0.1 and mh_distance_table[x][y] < 2.0:
        ...
        if user_table[x][y] == 0:
            if x+1 in similarUserDict:
                ...
                for i in similar_list:
                    manhattanList.update({mh_distance_table[x][i-1]:i})
                manhattanDict = {}
                manhattanDict = collections.OrderedDict(sorted(manhattanList.items(), key=lambda t: t[0]))
                for z in range(k):
                    rating = manhattanDict.itervalues().next()
