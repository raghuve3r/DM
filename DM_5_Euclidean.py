...
#table to fill euclidean distance between the users
eu_distance_table = [[0 for i in range(userCount)] for j in range(userCount)]

#filling the eucidean distance table between users
for x in range(userCount):
    for y in range(userCount):
        if x != y:
            num_array = numpy.array([user_table[x],numpy.array(user_table[y])])
            eu_distance_table[x][y] = float(str(pdist(num_array ,'Euclidean')).replace('[','').replace(']',''))

#finding the similar users for a particular user
for x in range(userCount):
    for y in range(userCount):
        if eu_distance_table[x][y] > 0.1 and eu_distance_table[x][y] < 3.0:
        ...
        if user_table[x][y] == 0:
            if x+1 in similarUserDict:
                ...
                for i in similar_list:
                    euclideanList.update({eu_distance_table[x][i-1]:i})
                euclideanDict = {}
                euclideanDict = collections.OrderedDict(sorted(euclideanList.items(), key=lambda t: t[0]))
                for z in range(k):
                    rating = euclideanDict.itervalues().next()
